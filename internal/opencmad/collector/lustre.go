...
func (l *lustreCollector) updateLustreHealth(ch chan<- *pb.CollectData) error {
	healthCheck, err := fpx.ReadAll(filepath.Join(l.healthCheckPath, "health_check"))
	if err != nil {
		return err
	}
	version, err := fpx.ReadAll(sysFilePath("fs/lustre/version"))
	if err != nil {
		return err
	}
	...
	for _, key := range []string{"max_dirty_mb", "memused", "memused_max"} {
		data, err := fpx.ReadAll(sysFilePath("fs/lustre/" + key))
		if err != nil {
			fields[key] = 0.0
