...
func (l *lustreCollector) updateClientState(ch chan<- *pb.CollectData) error {
	...
	for _, fp := range fps {
		clientStateData, err := fpx.ReadAll(fp)
		if err != nil {
			return err
		}
...
func (l *lustreCollector) updateLliteStats(ch chan<- *pb.CollectData) error {
