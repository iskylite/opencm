...
import (
	...
	"github.com/iskylite/opencm/pkg/fpx"
...
func (l *lustreCollector) updateTargetState(targetType string, ch chan<- *pb.CollectData) error {
	...
	for _, fp := range fps {
		numExports, err := fpx.ReadAll(filepath.Join(fp, "num_exports"))
		if err != nil {
			return err
		}
...
func (l *lustreCollector) updateTargetSize(ch chan<- *pb.CollectData) error {
	...
	for _, fp := range fps {
		...
		for _, tKey := range []string{"fstype", "mntdev", "nonrotational"} {
			tval, err := fpx.ReadAll(filepath.Join(fpDir, tKey))
			if err != nil {
				return err
			}
		...
		for _, fKey := range []string{"filesfree", "filestotal", "kbytesavail", "kbytestotal", "kbytesfree"} {
			fdata, err := fpx.ReadAll(filepath.Join(fpDir, fKey))
			if err != nil {
				return err
			}
...
func (l *lustreCollector) updateTargetJobStats(ch chan<- *pb.CollectData) error {
	...
	for _, fp := range fps {
		...
		jobStatsDataBytes, err := fpx.ReadAll(fp)
		if err != nil {
			return err
		}
...
func (l *lustreCollector) updateTargetQuotaSlave(ch chan<- *pb.CollectData) error {
	...
	for _, fp := range fps {
		...
		quotaSlaveDataBytes, err := fpx.ReadAll(fp)
		if err != nil {
			return err
		}
