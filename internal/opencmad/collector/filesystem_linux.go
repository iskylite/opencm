...
const (
	defFSTypesIncluded     = "^(ramfs|ext4)$"
	defMountPointsExcluded = "^/(dev|proc|boot|run/credentials/.+|sys|var/lib/docker/.+)($|/)"
)
...
func (c *filesystemCollector) GetStats() ([]filesystemStats, error) {
	...
	for _, labels := range mps {
		...
		if !c.includedFSTypesPattern.MatchString(labels.fsType) {
