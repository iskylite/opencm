...
func (c *zfsCollector) updatePoolStats(ch chan<- *transport.Data) error {
	...
	for _, zpoolPath := range zpoolPaths {
		...
		err = c.parsePoolProcfsFile(file, zpoolPath, func(poolName string, io map[string]float64) {
			ch <- c.constPoolMetric(rtime, poolName, io)
		})
		file.Close()
		if err != nil {
			return err
	...
	for _, zpoolPath := range zpoolStatePaths {
		...
		err = c.parsePoolStateFile(file, zpoolPath, func(poolName string, stateName string) {
			ch <- c.constPoolStateMetric(rtime, poolName, stateName)
		})
...
func (c *zfsCollector) parsePoolProcfsFile(reader io.Reader, zpoolPath string, handler func(string, map[string]float64)) error {
	scanner := bufio.NewScanner(reader)

	parseLine := false
	var fields []string
	var io map[string]float64
	var zpoolName string
	for scanner.Scan() {
		...
		if !parseLine && len(line) >= 12 && line[0] == "nread" {
			...
			io = make(map[string]float64, len(line))
			copy(fields, line)
			continue
		}
		if !parseLine {
		...
		for i, field := range fields {
			...
			if err != nil {
				return fmt.Errorf("could not parse expected integer value for %s: %w", field, err)
			}
			io[field] = float64(value)
		}
	}

	handler(zpoolName, io)
	return scanner.Err()
}
...
func (c *zfsCollector) parsePoolStateFile(reader io.Reader, zpoolPath string, handler func(string, string)) error {
	scanner := bufio.NewScanner(reader)
	scanner.Scan()
	...
	handler(zpoolName, actualStateName)
	return nil
}
